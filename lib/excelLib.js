...
export default async function excelInvoice(patient, invoice) {
  ...
  ws.getCell("E20").value = invoice.valore; //`€ ${invoice.valore}`; // Formattare come valuta
  ws.getCell("A20").value = invoice.testo;
  const value = Number.parseFloat(invoice.valore);
  const inps = value * 0.04;
  ws.getCell("F34").value = value - inps; //`€ ${value - inps}`;
  ws.getCell("F35").value = inps; //`€ ${inps}`;
  ws.getCell("F36").value = value; //`€ ${value}`;
