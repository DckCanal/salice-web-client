...
const createFileStructure = (ws) => {
  ...
  ws.getColumn(5).width = 10 + 0;
  ws.getColumn(6).width = 10 + 0;
  ...
  ws.getCell("A40").alignment = { horizontal: "center" };
  ...
  for (let i of ["A", "B", "C", "D", "E", "F"]) {
    let c;
    for (let j of [33, 36, 40]) {
      c = ws.getCell(`${i}${j}`);
      const s = j != 40 ? "hair" : "thin";
...
export default async function excelInvoice(patient, invoice) {
  ...
  ws.getCell("A40").value =
    "Iscritto all'elenco speciale ad esaurimento TSRM-PSTRP di Forlì-Cesena Rimini, iscrizione N. 123";
